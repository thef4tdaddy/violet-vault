...
lines.forEach(line => {
  ...
  const match = line.match(/^([^:]+)\((\d+),\d+\): error TS6133: 'React' is declared but its value is never read\.$/);
  if (!match) return;
  ...
  const usesReact = content.includes('React.') || content.includes('React[') || content.includes('React.createElement') || content.includes('React.cloneElement') || content.includes('React.Fragment');
