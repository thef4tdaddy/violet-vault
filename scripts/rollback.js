    ...
    if (fs.existsSync(this.rollbackDir)) {
      ...
      if (rollbacks.length > 0) {
        rollbacks.slice(0, 3).forEach(rollback => {
          try {
            const rollbackData = JSON.parse(fs.readFileSync(path.join(this.rollbackDir, rollback), 'utf8'));
            console.log(`  ${rollback} (${rollbackData.currentCommit.split(' ').slice(1).join(' ')})`);
          } catch (error) {
