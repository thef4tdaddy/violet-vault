...
const USE_GH_CLI = true; // Use GitHub CLI instead of API token

/**
 * Parse strict mode errors from results file
 */
function parseErrors() {
  const content = fs.readFileSync(STRICT_RESULTS_FILE, "utf-8");
...
function getAgentPrompt() {
  ...
  if (noteIndex !== -1) {
    result.splice(
      ...
      "git checkout -b fix-ts-strict-errors-batch-X",
      "",
      "# Work on fixes...",
      "# Commit changes...",
      "",
      "# Push when ready",
      "git push origin fix-ts-strict-errors-batch-X",
      "```",
      ""
    );
  }
 ...
 */
function groupErrorsIntoBatches(errorsByFile) {
  const batches = [];
  let currentBatch = {
    files: [],
    errors: [],
  ...
  for (const [file, errors] of sortedFiles) {
    ...
    if (
      currentBatch.errorCount + errors.length > ERRORS_PER_ISSUE &&
...
function formatErrorList(errors) {
  ...
  const lines = [];
  lines.push("## Files with Strict Mode Errors\n");
 ...
 * Create GitHub issue
 */
async function createGitHubIssue(batch, batchNumber, totalBatches, agentPrompt) {
  const title = `Fix TypeScript Strict Mode Errors (Batch ${batchNumber}/${totalBatches}) - ${batch.errorCount} errors in ${batch.files.length} files`;

  const body = [
    `# TypeScript Strict Mode Error Fixing - Batch ${batchNumber}/${totalBatches}`,
    "",
    ...
    "---",
    "",
    "*This issue was automatically generated from strict mode type checking results*",
  ].join("\n");
  ...
  try {
    ...
    const tempFile = path.join(__dirname, `../.temp-issue-body-${batchNumber}.md`);
 ...
 */
async function main() {
  console.log("📊 Parsing strict mode errors...");
  const { errorsByFile, allErrors } = parseErrors();
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      const issue = await createGitHubIssue(batch, i + 1, batches.length, agentPrompt);
...
if (
  ...
  process.argv[1]?.endsWith("generate-strict-mode-issues.js")
) {
  main().catch((error) => {
