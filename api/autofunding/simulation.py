...
def simulate_rule_execution(
    rules: list[AutoFundingRule], context: AutoFundingContext
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        executable_rules = [rule for rule in rules if should_rule_execute(rule, context)]
        ...
        for rule in sorted_rules:
            try:
                ...
                if rule_result.success and rule_result.amount > 0:
                    ...
                    if rule_result.error:
                        simulation.errors.append(
                            ErrorResult(ruleId=rule.id, ruleName=rule.name, error=rule_result.error)
                        )
                ...
                simulation.errors.append(
                    ErrorResult(ruleId=rule.id, ruleName=rule.name, error=error_message)
                )
...
def simulate_single_rule(
    rule: AutoFundingRule, context: AutoFundingContext, available_cash: float
) -> RuleResult:
    """
...
def plan_rule_transfers(rule: AutoFundingRule, total_amount: float) -> list[PlannedTransfer]:
    """
...
def calculate_transfer_impact(
    transfers: list[PlannedTransfer], context: AutoFundingContext
) -> dict[str, Any]:
    """
