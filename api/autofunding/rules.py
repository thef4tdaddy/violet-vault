...
def calculate_funding_amount(rule: AutoFundingRule, context: AutoFundingContext) -> float:
    """
...
def get_base_amount_for_percentage(rule: AutoFundingRule, context: AutoFundingContext) -> float:
    """
    ...
    if source_type == "unassigned":
        ...
        if rule.config.sourceId:
            envelope = next((e for e in envelopes if e.id == rule.config.sourceId), None)
...
def calculate_priority_fill_amount(rule: AutoFundingRule, context: AutoFundingContext) -> float:
    """
    ...
    target_envelope = next((e for e in envelopes if e.id == rule.config.targetId), None)
...
def sort_rules_by_priority(rules: list[AutoFundingRule]) -> list[AutoFundingRule]:
    ...
    def sort_key(rule: AutoFundingRule) -> tuple[int, str]:
