...
- **Frontend**: React (Vite) + Dexie (Local) + Firebase (E2EE Cloud).
- **Backend (Go)**: `/api/*.go` - Used for bug reporting and API proxies.
- **Backend (Python)**: `/api/*.py` - Used for analytic and paycheck prediction.
- **Data Flow**: Firebase (E2EE) ↔ Dexie (Local) ↔ TanStack Query ↔ React UI.
- **CRITICAL**: Sensitive budgeting data must remain decrytped only in the browser (E2EE).
...
- Workflow changes: commit on develop, then cherry-pick to main (workflows only run from main branch).

### Directory Structure (v2.0)

- `/src/` - React Frontend source code.
- `/api/` - Polyglot Serverless Functions (Go, Python, Node.js).
- `/src/services/` - Client-side business logic and Firebase orchestration.
- `/src/hooks/api/` - TanStack Query hooks for both Firebase and /api calls.
- `/src/domain/schemas/` - Zod validation schemas (used for frontend and /api responses).
- `/scripts/` - CI/CD and automation scripts (Full Salvo).

### Coding Standards (Full Salvo v2.0)

- **Python**: Use Ruff (linter/formatter) and Mypy (types).
- **Go**: Use golangci-lint and gofmt.
- **React**: Use ESLint, Prettier, and TSC.
- **ALL**: Run `./scripts/full_salvo.sh` locally before opening PRs.
- **Logger**: Always use `@/utils/common/logger` for frontend logging.
...
- Sentry session replay masks all text and blocks all media for privacy.

---

## 🧪 Testing Standards

- Use Vitest for unit tests (Frontend) and pytest/go test (Backend).
- Test files: `component/__tests__/Component.test.tsx` or `module.test.ts` / `file_test.go`.
...
- **CRITICAL**: All new code (Frontend & Backend) must have >80% code coverage.
- Every new file created **MUST** be accompanied by a corresponding test file.
- Run tests: `npm run test` or `npm run test:run` for CI.
- Focus on error handling and edge cases, not just happy paths.
- Test critical operations: import/export, backup/restore, cloud sync, CRUD mutations.
- Reference `COVERAGE_GAP_ANALYSIS.md` for test coverage priorities.
...
- **React Context**: Auth state (user, session, auth status) - **NOT** Zustand (v2.0+).
...
- Use Sentry for error tracking: `captureError(error, context)` from `@/utils/common/sentry`.
- Error boundaries should use Sentry ErrorBoundary for production error tracking.
...
- Use Sentry for performance monitoring: custom spans/transactions for critical operations.
- Track performance metrics for queries, data operations, sync, and backup/restore.
...
- For workflow files: commit separately on develop, cherry-pick to main (workflows must be on main to run)
...
Note: Co-Authored-By line is optional and should only be added when explicitly requested
```
...
## 🚫 Prohibited Patterns & ESLint Policy

### Prohibited

- 🚫 **NO 'any' types** - EVER - use proper types or Zod schemas.
- 🚫 **NO relative imports** - ALWAYS use '@' path aliases.
- 🚫 **NO PRs without >80% test coverage** for new files (Frontend & Backend).
