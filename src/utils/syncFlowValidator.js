...
export const validateAllSyncFlows = async () => {
  ...
  try {
    ...
    const validations = [
      ...
      { name: "Bill in sync", test: syncData.bills.some((b) => b.id === testData.bill.id) },
      { name: "Debt in sync", test: syncData.debts.some((d) => d.id === testData.debt.id) },
      {
        name: "Metadata synced",
        test: syncData.unassignedCash === 500 && syncData.actualBalance === 2000,
      },
      { name: "Timestamp valid", test: !isNaN(syncData.lastModified) && syncData.lastModified > 0 },
    ];
  ...
  try {
    ...
    const lifecycleChecks = [
      { name: "Initial state valid", test: typeof initialStatus.isRunning === "boolean" },
