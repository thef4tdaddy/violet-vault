...
const useEnvelopeSystem = () => {
  ...
  const spendFromEnvelope = useCallback((envelopeId, amount, description = 'Spending') => {
  ...
  const transferBetweenEnvelopes = useCallback((fromEnvelopeId, toEnvelopeId, amount) => {
    ...
    setEnvelopes(updatedEnvelopes);
  }, [envelopes, unassignedCash, setEnvelopes, setUnassignedCash, spendFromEnvelope]);

  // Process paycheck allocation to envelopes
  const allocatePaycheckToEnvelopes = useCallback((paycheckAmount, allocationMode = 'allocate') => {
  ...
  const getEnvelopeStatus = useCallback((envelopeId) => {
    const envelope = getEnvelopeById(envelopeId);
    if (!envelope) return null;

    const percentFilled = envelope.biweeklyAllocation > 0 
