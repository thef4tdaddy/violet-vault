...
const ReportExporter = ({
  ...
  const exportToPDF = async () => {
    ...
    if (exportOptions.includeCharts) {
      ...
      pdf.setFont("helvetica", "normal");
      pdf.text(
        "Charts would be embedded here in full implementation.",
        margin,
        yPosition,
      );
    }
  ...
  const exportToCSV = async () => {
    ...
    if (analyticsData?.categoryBreakdown) {
      Object.entries(analyticsData.categoryBreakdown).forEach(
        ([category, data]) => {
          csvData.push([
            category,
            "Category",
  ...
  const handleTemplateSelect = (template) => {
    const templateOptions = {
      ...
      includeCharts:
        template.includes.includes("charts") ||
        template.includes.includes("budget_charts"),
