...
const BillEnvelopeFundingInfo = memo(
  ({ envelope, bills = [], showDetails = false }) => {
    ...
    return (
      <div
        className={`${status.bgColor} border border-${status.color}-200 rounded-lg p-3`}
      >
        {/* Header with status */}
        ...
        <div className="text-xs">
          ...
          {remainingToFund <= 0 && currentBalance > nextBill?.amount && (
            <div className="text-green-600 font-medium">
              ${(currentBalance - (nextBill?.amount || 0)).toFixed(2)} surplus
              available
            </div>
          )}
        </div>
        ...
        {showDetails && (
          <div className="mt-3 pt-2 border-t border-gray-200">
            <div className="grid grid-cols-2 gap-2 text-xs mb-2">
              ...
              {upcomingBillsAmount > 0 && (
                <div className="flex items-center space-x-1">
                  ...
                  <span className="text-gray-600">
                    ${upcomingBillsAmount.toFixed(2)} due (30d)
                  </span>
                </div>
              )}
            </div>
            ...
            {/* Overfunding alert */}
            {currentBalance > targetMonthlyAmount &&
              targetMonthlyAmount > 0 && (
                <div className="bg-green-50 p-2 rounded text-xs">
                  ...
                  <div className="text-green-600">
                    This envelope is{" "}
                    {Math.round((currentBalance / targetMonthlyAmount) * 100)}%
                    funded for the month
                  </div>
                </div>
              )}
