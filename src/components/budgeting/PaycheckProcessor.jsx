...
const PaycheckProcessor = ({
  ...
  const calculateAllocation = () => {
    ...
    envelopes.forEach((envelope) => {
      ...
      if (envelope.envelopeType === "variable" && envelope.monthlyBudget) {
        // Variable expense envelopes: calculate biweekly portion of monthly budget
        const biweeklyBudget = envelope.monthlyBudget / 2.17; // ~2.17 biweekly periods per month
        needed = Math.max(0, biweeklyBudget - envelope.currentBalance);
      } else {
        // Bill envelopes: use existing biweekly allocation logic
        needed = Math.max(
          0,
                ...
                <label className="glassmorphism flex items-start space-x-4 p-6 border-2 border-white/20 rounded-2xl cursor-pointer hover:border-purple-300 transition-all">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">
                      Fill bill envelopes (biweekly needs) and variable
                      envelopes (monthly budget), then put leftovers in
                      unassigned cash
                    </p>
                  </div>
                </label>
