...
const SyncHealthDashboard = ({ isOpen, onClose }) => {
  ...
  const successRate = ((healthData.metrics.successfulSyncs / (healthData.metrics.successfulSyncs + healthData.metrics.failedSyncs)) * 100) || 100;
      ...
      <div className="bg-white rounded-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <div className={`p-3 rounded-full bg-${statusColor}-100`}>
              {renderIcon(statusIcon, { className: `h-6 w-6 text-${statusColor}-600` })}
            </div>
            <div>
              <h2 className="text-xl font-black text-black uppercase tracking-wider">
                <span className="text-2xl">S</span>YNC <span className="text-2xl">H</span>EALTH <span className="text-2xl">D</span>ASHBOARD
              </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-purple-100/40 backdrop-blur-sm rounded-lg p-4 border-2 border-black">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-black text-${successRate > 95 ? 'green' : successRate > 90 ? 'yellow' : 'red'}-600`}>
                  {successRate.toFixed(1)}%
                </p>
              </div>
              {renderIcon('TrendingUp', { className: 'h-8 w-8 text-purple-300' })}
            </div>
          </div>
          ...
          <div className="bg-purple-100/40 backdrop-blur-sm rounded-lg p-4 border-2 border-black">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-black text-${healthData.metrics.averageSyncTime < 5000 ? 'green' : healthData.metrics.averageSyncTime < 10000 ? 'yellow' : 'red'}-600`}>
          ...
          <div className="bg-purple-100/40 backdrop-blur-sm rounded-lg p-4 border-2 border-black">
            <div className="flex items-center justify-between">
              ...
              {renderIcon('BarChart3', { className: 'h-8 w-8 text-purple-300' })}
            </div>
          </div>
          ...
          <div className="bg-purple-100/40 backdrop-blur-sm rounded-lg p-4 border-2 border-black">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-black text-${healthData.metrics.consecutiveFailures > 0 ? 'red' : 'green'}-600`}>
                  {healthData.metrics.consecutiveFailures}
                </p>
              </div>
              {renderIcon('AlertTriangle', { className: 'h-8 w-8 text-purple-300' })}
            </div>
          </div>
        </div>
        ...
        {(healthData.issues.length > 0 || healthData.status !== 'healthy') && (
          <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4 mb-6">
            <div className="flex items-start space-x-3">
              {renderIcon('AlertTriangle', { className: 'h-5 w-5 text-red-600 mt-1' })}
              <div className="flex-1">
                <h3 className="font-black text-red-800 text-sm uppercase mb-2">
                  <span className="text-base">S</span>YNC <span className="text-base">I</span>SSUES <span className="text-base">D</span>ETECTED
                </h3>
          ...
          <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4 mb-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-700">All sync operations are performing normally. No issues detected.</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-purple-100/40 backdrop-blur-sm rounded-lg p-4 border-2 border-black">
          <h3 className="font-black text-black text-sm uppercase mb-4">
            <span className="text-base">R</span>ECENT <span className="text-base">S</span>YNC <span className="text-base">H</span>ISTORY
          </h3>
            ...
            <div className="space-y-2">
              {healthData.recentSyncs.slice(0, 5).map((sync, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    {renderIcon(
                      sync.success ? 'CheckCircle' : 'XCircle',
                      { 
                        className: `h-4 w-4 ${
                          sync.success ? 'text-green-600' : 'text-red-600'
                        }`
                      }
                    )}
                    <div>
                      ...
                      {sync.error && (
                        <p className="text-xs text-red-600">{sync.error.message}</p>
                      )}
                    </div>
                  </div>
