  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        {/* Connected Envelopes */}
        {connectedEnvelopes.length > 0 && (
          <div className="bg-indigo-50 rounded-xl p-4 mb-6">
            <h4 className="font-medium text-indigo-900 mb-3">Connected Envelopes</h4>
            <div className="space-y-2">
              {connectedEnvelopes.map((envelope) => (
                <div
                  key={envelope.id}
                  className="flex justify-between items-center bg-white rounded-lg p-3"
                >
                  <div>
                    <p className="font-medium text-indigo-900">{envelope.name}</p>
