...
export const useAutoFundingRules = (initialRules = []) => {
  ...
  const getFilteredRules = useCallback((filters = {}) => {
    try {
  ...
  const getExecutableRules = useCallback((context) => {
    try {
  ...
  const bulkToggleRules = useCallback((ruleIds, enabled) => {
    try {
