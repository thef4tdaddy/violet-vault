...
describe('useImportData', () => {
  it('should import data successfully', async () => {
    useAuth.mockReturnValue({ currentUser: { userName: 'testuser', budgetId: '123' } });
    const showSuccessToast = vi.fn();
    useToastHelpers.mockReturnValue({ showSuccessToast, showErrorToast: vi.fn() });
    useConfirm.mockReturnValue(vi.fn().mockResolvedValue(true));
    readFileContent.mockResolvedValue(JSON.stringify({ envelopes: [] }));
    validateImportedData.mockReturnValue({ validatedData: { envelopes: [] }, hasBudgetIdMismatch: false });
    ...
    expect(showSuccessToast).toHaveBeenCalledWith('Import complete! Data synced to cloud successfully.');
  });
});