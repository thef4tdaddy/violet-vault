...
describe("useSyncHealthMonitor", () => {
  ...
  it("should auto-refresh when autoRefresh is true", () => {
    ...
    expect(mockGetHealthStatus).toHaveBeenCalledTimes(1); // Initial call only (no redundant refresh in effect)

    act(() => {
      vi.advanceTimersByTime(5000);
    });

    expect(mockGetHealthStatus).toHaveBeenCalledTimes(2); // Initial + first interval refresh

    act(() => {
      vi.advanceTimersByTime(5000);
    });

    expect(mockGetHealthStatus).toHaveBeenCalledTimes(3); // Initial + two interval refreshes
  });
  ...
  it("should restart interval when autoRefresh changes", () => {
    ...
    expect(mockGetHealthStatus).toHaveBeenCalledTimes(1); // Initialization only

    act(() => {
      vi.advanceTimersByTime(5000);
    });

    expect(mockGetHealthStatus).toHaveBeenCalledTimes(2); // Auto-refresh working
  });
  ...
  it("should restart interval when interval changes", () => {
    ...
    expect(mockGetHealthStatus).toHaveBeenCalledTimes(3); // New interval working
  });
