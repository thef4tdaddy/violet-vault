...
export const initializeSplitsHandler = (
  ...
  try {
    ...
    logger.debug("Initializing transaction splits", {
      ...
      hasMetadata: !!(transaction as Transaction & { metadata?: { items?: unknown } }).metadata?.items,
    });
...
export const autoBalanceHandler = (
  ...
  try {
    ...
    setErrors(["Failed to auto-balance splits: " + (error as Error).message]);
  }
};
...
export const distributeEvenlyHandler = (
  ...
  try {
    ...
    setErrors(["Failed to split evenly: " + (error as Error).message]);
  }
};
