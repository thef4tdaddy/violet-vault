...
export const usePaycheckOperations = () => {
  const handleDeletePaycheck = useCallback(
    async (paycheckId, paycheckHistory) => {
      try {
        ...
        const paycheckToDelete = validatePaycheckDeletion(
          paycheckId,
          paycheckHistory,
        );
        ...
        // Invalidate caches
        const { queryClient, queryKeys } = await import(
        ...
        logger.info(
          "Paycheck deleted successfully with proper balance reversal",
          {
            paycheckId,
            actualBalanceChange:
              balances.newActualBalance - balances.currentActualBalance,
            unassignedCashChange:
              balances.newUnassignedCash - balances.currentUnassignedCash,
          },
        );
      } catch (error) {
        logger.error(
