...
export class VioletVaultDB extends Dexie {
  ...
  budget!: Table<BudgetMetadata | BudgetData, string>;
  ...
  constructor() {
    ...
    const addTimestampHooks = (table: Table<BaseEntity, any>) => {
        ...
        try {
          ...
          if (
            ...
            Object.keys(obj).forEach((key) => {
              try {
                extensibleObj[key] = (obj as any)[key];
              } catch (copyError: any) {
                logger.warn(
            ...
            try {
              ...
              try {
                ...
                Object.keys(extensibleObj).forEach((key) => {
                  try {
                    (obj as any)[key] = extensibleObj[key];
                  } catch (setError: any) {
                    logger.warn(
  ...
  // Envelope queries
  async getEnvelopesByCategory(category: string, includeArchived: boolean = false): Promise<Envelope[]> {
    ...
    if (!result) {
      if (includeArchived) {
        ...
        result = await this.envelopes
          ...
          .equals([category, false as any] as any)
          .toArray();
      }
  ...
  async getTransactionsByDateRange(startDate: Date | string, endDate: Date | string): Promise<Transaction[]> {
  ...
  async getTransactionsByEnvelope(envelopeId: string, dateRange?: DateRange): Promise<Transaction[]> {
    if (dateRange) {
      return this.transactions
  ...
  async getTransactionsByType(type: string, dateRange?: DateRange): Promise<Transaction[]> {
    if (dateRange) {
      return this.transactions
        .where("[type+date]")
  ...
  async getUpcomingBills(daysAhead: number = 30): Promise<Bill[]> {
    ...
    return this.bills
      ...
      .between([today, false as any] as any, [futureDate, false as any] as any, true, true)
      .toArray();
  }
  ...
  async getOverdueBills(): Promise<Bill[]> {
    ...
    return this.bills
      ...
      .between([false as any, new Date(0)] as any, [false as any, today] as any, true, false)
      .toArray();
  }
  ...
  async getPaidBills(dateRange?: DateRange): Promise<Bill[]> {
    if (dateRange) {
      return this.bills
        .where("[isPaid+dueDate]")
        .between([true as any, dateRange.start] as any, [true as any, dateRange.end] as any, true, true)
        .toArray();
    }
  ...
  async getActiveSavingsGoals(): Promise<SavingsGoal[]> {
    return this.savingsGoals
      ...
      .equals([false as any, false as any] as any)
      .toArray();
  }
  ...
  async getSavingsGoalsByPriority(priority: string): Promise<SavingsGoal[]> {
  ...
  async getUpcomingDeadlines(daysAhead: number = 30): Promise<SavingsGoal[]> {
    ...
    return this.savingsGoals
      ...
      .between([new Date(), false as any] as any, [futureDate, false as any] as any, true, true)
      .toArray();
  }
  ...
  async getPaychecksByDateRange(startDate: Date | string, endDate: Date | string): Promise<PaycheckHistory[]> {
    return this.paycheckHistory
      .where("date")
      .between(startDate, endDate, true, true)
      .toArray();
  ...
  async batchUpdate(updates: BulkUpdateOperation[]): Promise<any[]> {
  ...
  async getAnalyticsData(dateRange: DateRange, includeTransfers: boolean = false): Promise<AnalyticsData> {
    const transactions = await this.getTransactionsByDateRange(
      dateRange.start,
      dateRange.end,
    );

    const filteredTransactions = includeTransfers
      ? transactions
  ...
  async setCachedValue(key: string, value: any, ttl: number = 300000, category: string = "general"): Promise<void> {
  ...
  async optimizeDatabase(): Promise<void> {
    ...
    if (auditCount > 1000) {
      ...
      await this.auditLog.bulkDelete(oldIds as any);
    }
  }
  ...
  async createBudgetChanges(changes: BudgetChange[]): Promise<any> {
...
// Utility functions
export const getEncryptedData = async (): Promise<BudgetData | null> => {
  try {
    return await budgetDb.budget.get("budgetData") as BudgetData | undefined || null;
  } catch {
    return null;
  }
};

export const setEncryptedData = async (data: Partial<BudgetData>): Promise<void> => {
  try {
...
export const getBudgetMetadata = async (): Promise<BudgetMetadata | null> => {
  try {
    return await budgetDb.budget.get("metadata") as BudgetMetadata | undefined || null;
  } catch {
    return null;
  }
};

export const setBudgetMetadata = async (metadata: Partial<BudgetMetadata>): Promise<void> => {
  try {
    await budgetDb.budget.put({
      id: "metadata",
      lastModified: Date.now(),
...
export const setUnassignedCash = async (amount: number): Promise<void> => {
  const currentMetadata = (await getBudgetMetadata()) || {} as Partial<BudgetMetadata>;
...
export const queryHelpers = {
  ...
  // Analytics helpers
  getAnalyticsData(dateRange: DateRange, includeTransfers?: boolean): Promise<AnalyticsData> {
