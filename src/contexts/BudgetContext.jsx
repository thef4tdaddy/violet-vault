    ...
    case actionTypes.LOAD_DATA: {
      ...
      if (validatedPayload.bills.length > 0) {
        ...
        if (billsToMigrate.length > 0) {
          console.log(`🔄 Migrating ${billsToMigrate.length} bills to unified structure`);
          ...
          const migratedBills = billsToMigrate
            ...
            .map((bill) => ({
              ...
              date: bill.date || bill.createdAt || new Date().toISOString().split("T")[0],
      ...
      if (encryptionKey && currentUser && budgetId) {
        ...
        if (savedData) {
          try {
            ...
            logger.error("❌ Failed to load/decrypt data from localStorage.", error);
