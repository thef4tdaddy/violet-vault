...
export const BudgetProvider = ({ children, encryptionKey, currentUser, budgetId, salt }) => {
  // Test if component body runs
  logger.budgetSync("BudgetProvider COMPONENT BODY - should fire on every render", {
  ...
  // DIRECT SYNC INITIALIZATION - bypass useEffect since it's not working
  React.useMemo(() => {
    if (encryptionKey && currentUser && budgetId) {
      console.log("🚀 DIRECT SYNC INIT - bypassing useEffect");
      logger.budgetSync("Direct sync initialization", {
        budgetId,
        userName: currentUser.userName,
        timestamp: new Date().toISOString(),
      });

      // Initialize sync directly
      setTimeout(async () => {
        try {
          await firebaseSync.initialize(budgetId, encryptionKey, currentUser);
          console.log("✅ Direct sync initialization complete");
        } catch (error) {
          console.error("❌ Direct sync initialization failed:", error);
        }
      }, 100);
    }
