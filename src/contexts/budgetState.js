...
export const budgetReducer = (state, action) => {
  switch (action.type) {
    case actionTypes.SET_ENVELOPES:
      return { ...state, envelopes: Array.isArray(action.payload) ? action.payload : [] };
    ...
    case actionTypes.UPDATE_ENVELOPE:
      return { ...state, envelopes: state.envelopes.map((env) => (env.id === action.payload.id ? action.payload : env)) };
    case actionTypes.DELETE_ENVELOPE:
      return { ...state, envelopes: state.envelopes.filter((env) => env.id !== action.payload) };
    case actionTypes.ADD_BILL:
      return { ...state, bills: [...state.bills, action.payload] };
    case actionTypes.UPDATE_BILL:
      return { ...state, bills: state.bills.map((bill) => (bill.id === action.payload.id ? action.payload : bill)) };
    ...
    case actionTypes.LOAD_DATA: {
      const validatedPayload = {
        ...
        supplementalAccounts: Array.isArray(action.payload.supplementalAccounts) ? action.payload.supplementalAccounts : [],
        transactions: Array.isArray(action.payload.transactions) ? action.payload.transactions : [],
        allTransactions: Array.isArray(action.payload.allTransactions) ? action.payload.allTransactions : [],
        paycheckHistory: Array.isArray(action.payload.paycheckHistory) ? action.payload.paycheckHistory : [],
      };
      const seen = new Set();
