...
class TypedChunkedSyncServiceImpl implements TypedChunkedSyncService {
  ...
  async initialize(budgetId: string, encryptionKey: string): Promise<void> {
    if (!isString(budgetId) || !isString(encryptionKey)) {
      throw new Error('Invalid initialization parameters: budgetId and encryptionKey must be strings');
    }

    if (budgetId.length === 0 || encryptionKey.length === 0) {
      throw new Error('Invalid initialization parameters: budgetId and encryptionKey cannot be empty');
    }
  ...
  async saveToCloud<T extends SafeUnknown>(
    ...
    return this.operationWrapper.executeWithRetry(async () => {
      // Validate input data
      if (data === undefined) {
    ...
    return this.operationWrapper.execute(async () => {
      ...
      if (isObject(data)) {
        ...
        const hasValidStructure = (
          ('transactions' in obj && (obj.transactions === null || Array.isArray(obj.transactions))) &&
          ('envelopes' in obj && (obj.envelopes === null || Array.isArray(obj.envelopes)))
        );
  ...
  public getChunkingInfo<T extends SafeUnknown>(data: T): {
