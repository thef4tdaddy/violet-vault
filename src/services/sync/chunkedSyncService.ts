...
import { validateCloudSyncUserSafe, formatSyncValidationErrors } from "@/domain/schemas/sync";
...
class ChunkedSyncService implements IChunkedSyncService {
  ...
  async calculateSize(data: unknown) {
    try {
      ...
      const { optimizedSerialization } = await import("@/utils/security/optimizedSerialization.js");
  ...
  async saveToCloud<T extends SafeUnknown>(
    ...
    }); // Fixed: removed "saveToCloud" argument from syncOperationWrapper.execute
  }
  ...
  async loadFromCloud<T extends SafeUnknown>(): Promise<TypedResponse<T | null>> {
    return syncOperationWrapper.execute(async () => {
      ...
      try {
        ...
        return reconstructedData as T; // Fixed: added type assertion
      } catch (error) {
    ...
    }); // Fixed: removed "loadFromCloud" argument
  }
