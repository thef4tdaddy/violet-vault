...
export interface SyncResult {
  success: boolean;
  direction?: "fromFirestore" | "toFirestore" | "none";
  error?: string;
  reason?: string;
...
class CloudSyncService {
   ...
   * TODO (v2.0): Abstract this to support multiple backend protocols (HTTP/REST, GraphQL, WebSocket).
   * Currently locked to Firebase/Firestore implementation.
   */
  async forceSync(): Promise<SyncResult> {
    if (this.isSyncing) {
      logger.warn("🔄 Sync already in progress, skipping.");
      return { success: false, reason: "Sync in progress", direction: "none" };
    }
    ...
    try {
      ...
      if (syncDecision.direction === "fromFirestore") {
        ...
        const loadResult = await chunkedSyncService.loadFromCloud<DexieData>();
        if (loadResult.success && loadResult.data) {
          // Save the loaded data to Dexie
          await this.saveToDexie(loadResult.data);
          ...
          result = {
            success: false,
            error: loadResult.error?.message || "No cloud data found",
          };
        }
      } else {
      ...
      if (result?.success) {
        ...
        logger.error("❌ Chunked sync failed:", errorMessage);
        ...
        syncHealthMonitor.recordSyncFailure(syncId, new Error(errorMessage), {
          backupId,
