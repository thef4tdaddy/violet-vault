  ...
  async saveToDexieAndInvalidate(data) {
    ...
    try {
      ...
      if (data.budgetCommits?.length > 0) {
        ...
        logger.debug(`💾 Saving ${data.budgetCommits.length} budget commits to Dexie`);
      }

      if (data.budgetChanges?.length > 0) {
        promises.push(budgetDb.budgetChanges.bulkPut(data.budgetChanges));
        logger.debug(`💾 Saving ${data.budgetChanges.length} budget changes to Dexie`);
      }
      ...
      if (data.budgetBranches?.length > 0) {
        ...
        logger.debug(`💾 Saving ${data.budgetBranches.length} budget branches to Dexie`);
      }

      if (data.budgetTags?.length > 0) {
        promises.push(budgetDb.budgetTags.bulkPut(data.budgetTags));
        logger.debug(`💾 Saving ${data.budgetTags.length} budget tags to Dexie`);
      }
