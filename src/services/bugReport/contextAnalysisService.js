...
export class ContextAnalysisService {
  ...
  static getActiveModals() {
    const modals = document.querySelectorAll(
      '[role="dialog"], .modal, [data-modal], .overlay'
    );
  ...
  static getActiveDrawers() {
    const drawers = document.querySelectorAll(
      '.drawer, .sidebar, [data-drawer], [data-sidebar]'
    );
  ...
  static getButtonStates() {
    ...
    return Array.from(buttons).slice(0, 10).map(button => ({ // Limit to first 10
  ...
  static getInputStates() {
    ...
    return Array.from(inputs).slice(0, 10).map(input => ({ // Limit to first 10
      name: input.name,
      type: input.type,
    ...
    return Array.from(notifications)
      ...
      .map(el => ({
        ...
        type: el.className.includes('error') ? 'error' : 
              el.className.includes('success') ? 'success' :
  ...
  static getListData() {
    ...
    return Array.from(lists).slice(0, 5).map(list => ({
